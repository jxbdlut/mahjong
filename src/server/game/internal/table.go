...
func (t *Table) CheckHu(disCard mahjong.DisCard) bool {
...
func (t *Table) DisCard(disCard mahjong.DisCard) {
	pos, err := t.GetPlayerIndex(disCard.FromUid)
	if err != nil {
		log.Error("next_pos err:%v", err)
		return
...
func (t *Table) GetNeedHunInSub(sub_cards []int32, min_need *Ting, max_need *Ting) *Ting {
	...
	if len_sub_cards == 0 {
		...
		return GetMin(min_ting.AddNum(2), max_ting)
	} else if len_sub_cards == 2 {
		m, v0, v1 := sub_cards[0]/100, sub_cards[0]%10, sub_cards[1]%10
		if m == 4 {
			if v0 == v1 {
				return GetMin(min_ting.AddNum(1), max_ting)
			}
		} else if v1-v0 < 3 {
			return GetMin(min_ting.AddNum(1), max_ting)
		}
		...
		for i := 1; i < len_sub_cards; i++ {
			ting1 := min_ting.Copy()
			if ting1.AddTing(t.GetModNeedNum(len_sub_cards-3, false)).BiggerOrEqual(max_ting) {
				break
			}
			v1 := sub_cards[i] % 10
			// 13444   134不可能连一起
		...
		v1 := sub_cards[1] % 10
		ting2 := min_ting.Copy()
		if ting2.AddTing(t.GetModNeedNum(len_sub_cards-2, false)).AddNum(1).Smaller(max_ting) {
			if m == 4 {
				if v0 == v1 {
					tmp_cards := mahjong.Copy(sub_cards[2:])
					max_ting = t.GetNeedHunInSub(tmp_cards, min_ting.AddNum(1), max_ting)
				}
			} else {
				for i := 1; i < len_sub_cards; i++ {
					ting4 := min_ting.Copy()
		...
		if min_ting.AddTing(t.GetModNeedNum(len_sub_cards-1, false)).AddNum(2).Smaller(max_ting) {
			...
			max_ting = t.GetNeedHunInSub(tmp_cards, min_ting.AddNum(2), max_ting)
		}
	}
	return max_ting
}
	...
	for i := 0; i < len_cards; i++ {
		if i == len_cards-1 { // 如果是最后一张牌
			...
			if t.rule.IsJiang(cards_copy[i]) {
				...
				min_ting = GetMin(min_ting, t.GetNeedHunInSub(tmp_cards, Minting(), MaxTing()).Copy().AddNum(1))
			} else {
				min_ting = GetMin(min_ting, t.GetNeedHunInSub(tmp_cards, Minting(), MaxTing()).Copy().AddNum(2))
			}
		} else {
			...
			if cards_copy[i]%10 != cards_copy[i+1]%10 {
				...
				if t.rule.IsJiang(tmp_cards[i]) {
					tmp_cards = t.Remove(tmp_cards, cards_copy[i], 0, 0)
					min_ting = GetMin(min_ting, t.GetNeedHunInSub(tmp_cards, Minting(), MaxTing()).AddNum(1))
...
//				} else {
//					min_need_num = min(min_need_num, t.GetNeedHunInSub(tmp_cards, 0, 4)+2)
//				}
//			}
//			if cards_copy[i]%10 != cards_copy[i+1]%10 {
//				tmp_cards := mahjong.Copy(cards_copy)
...
//func (t *Table) IsHu(cards []int32, hun_num int) bool {
//	cards_copy := mahjong.Copy(cards)
//	len_cards := len(cards_copy)
//	if len_cards == 0 {
//		if hun_num >= 2 {
...
//					}
//				} else {
//					if t.GetNeedHunInSub(tmp_cards, 0, 4) <= hun_num-2 {
//						return true
//					}
//				}
//			}
...
func (t *Table) HasDuiJiang(cards []int32) bool {
	var cache []int32
	for _, card := range cards {
		if mahjong.Contain(cache, card) {
			continue
		}
		cache = append(cache, card)
		if mahjong.Count(cards, card) >= 2 && t.rule.IsJiang(card) {
			return true
		}
	}
	return false
...
//				continue
//			}
//			if ok := result[2]; ok != nil && need_hun_arr[i] == 0 && !t.HasDuiJiang(separate_results[i+1]){
//				continue
//			}
//			tmp_cards := mahjong.Copy(separate_results[i+1])
//			tmp_cards = append(tmp_cards, card)
//			mahjong.SortCards(tmp_cards, t.hun_card)
//			if t.IsHu(tmp_cards, need_hun_with_eye_arr[i]-1) {
//				result[card] = card
//			}
//		}
//		for j := 0; j < 4; j++ {
...
func (t *Table) Play() {
	...
	for len(t.left_cards) > 10 && t.win_player == nil && len(t.players) == 4 {
		player := t.players[t.play_turn]
		t.play_turn = (t.play_turn + 1) % len(t.players)
		discard := player.Draw(mahjong.DisCard_Mo)
		if discard.Card != 0 {
			if discard.DisType == mahjong.DisCard_BuGang {
				t.DisCard(discard)
				discard = player.Draw(mahjong.DisCard_SelfGang)
			}
			t.DisCard(discard)
			t.round += 1
		} else {
